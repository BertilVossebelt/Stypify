...
public class GroupProvider : BaseProvider
{
    ...
    public List<Dictionary<string, object>>? GetLessonsWithCompleteAttribute(int groupId)
    {
        var cmd = GetSqlCommand();
        cmd.CommandText = "SELECT l.id, l.name, l.teacher_id, l.image, ul.completed FROM [Lesson] l JOIN [Group_Lesson] gl ON l.id = gl.lesson_id LEFT JOIN [User_Lesson] ul ON l.id = ul.lesson_id WHERE gl.group_id = @groupId";
